...
def mlp(x, hidden_sizes=(32,), activation=tf.tanh, output_activation=None):
    ...
    # Iterate over all layers except last
    for hidden in hidden_sizes[:-1]:
        x = tf.layers.dense(x, units=hidden, activation=activation)
    # Return output layer with output activation
...
def mlp_gaussian_policy(x, a, hidden_sizes, activation, output_activation, action_space):
    ...
    std = tf.exp(log_std)
    # tf.random.normal represents a vector of noise from a spherical Gaussian
    pi = mu + std * tf.random.normal(tf.shape(mu))

    # The log-likelihood as computed in exercise 1
